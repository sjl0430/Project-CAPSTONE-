...
class _ScanResultTileState extends State<ScanResultTile> {
  ...
  Widget _buildTitle(BuildContext context) {
    // 장치 이름이 비어 있지 않고 검색어와 일치하면 타일을 표시
    if (widget.result.device.platformName.isNotEmpty &&
        widget.result.device.platformName.toLowerCase().contains(widget.lastSearchQuery.toLowerCase())) {
      return Column(
    ...
    // 장치 이름이 비어 있거나 검색어와 일치하지 않으면 빈 타일을 반환
    return SizedBox.shrink(); // 빈 공간을 반환하여 이 항목을 표시하지 않음
  }
  ...
  Widget build(BuildContext context) {
    ...
    // 장치 이름이 비어있거나 검색어와 일치하지 않으면 해당 스캔 결과를 표시하지 않음
    if (widget.result.device.platformName.isEmpty ||
        !widget.result.device.platformName.toLowerCase().contains(widget.lastSearchQuery.toLowerCase())) {
      return SizedBox.shrink(); // 빈 공간을 반환하여 이 항목을 표시하지 않음
    }
